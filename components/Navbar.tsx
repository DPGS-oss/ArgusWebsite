  ...
  return (
    <nav
      className={`fixed inset-x-0 top-0 z-50 border-b transition ${
        scrolled
          ? "border-lead/20 bg-abyss/95 shadow-lg shadow-black/20 backdrop-blur"
          : "border-transparent bg-abyss/80 backdrop-blur"
      }`}
    >
      <div className="container-page flex h-16 items-center justify-between">
        ...
        <div
          className={`absolute left-0 right-0 top-16 flex flex-col gap-4 border-b border-lead/20 bg-midnight p-4 md:static md:flex md:flex-row md:items-center md:border-0 md:bg-transparent md:p-0 ${
        ...
        >
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-sm text-silver transition hover:text-starlight"
        ...
        >
          <span className="block h-0.5 w-6 bg-starlight" />
          <span className="block h-0.5 w-6 bg-starlight" />
          <span className="block h-0.5 w-6 bg-starlight" />
        </button>
      </div>
